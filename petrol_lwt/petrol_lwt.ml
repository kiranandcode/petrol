...
module VersionedSchema = struct
  ...
  let order_by_version ls =
    List.sort (fun (vl, _) (vr, _) -> compare_version vl vr) ls
  ...
  let init ?(migrations = []) version ~name =
    ...
    let version_table_name, Expr.[ version_table_field ] =
      StaticSchema.declare_table version_db
        ~name:("petrol_" ^ name ^ "_version_db")
        Schema.
          [
            field
              ~constraints:[ primary_key (); not_null () ]
              "version" ~ty:Type.TEXT;
          ]
    in
    {
      version;
  ...
  let declare_table t ?since ?(constraints : _ list = []) ?(migrations = [])
      ~name tbl =
    List.iter Schema.ensure_table_constraint constraints;
    let id = Hashtbl.length t.tables in
    let migrations = order_by_version migrations in
    Hashtbl.add t.tables id
      (MkTable (id, name, since, migrations, tbl, constraints));
    let rec to_table : 'a. string -> 'a Schema.table -> 'a Expr.expr_list =
     fun name (type a) (table : a Schema.table) : a Expr.expr_list ->
      match table with
      | [] -> []
  ...
  let get_current_version t con =
    ...
    let* res =
      ...
      |> Request.make_zero_or_one |> find_opt con
    in
    match res with
    | None ->
    ...
    | Some (old_version, ()) ->
        let old_version =
          old_version |> String.split_on_char '.' |> List.map int_of_string
        in
  ...
  let migrations_needed t ((module DB : Caqti_lwt.CONNECTION) as conn) =
    ...
    | _ -> Lwt.return_error (`Newer_version_than_supported current_version)
  ...
  let initialise t ((module DB : Caqti_lwt.CONNECTION) as conn) =
    ...
    | 0 ->
        ...
        Lwt_result.return ()
    | v when v < 0 ->
        (* version on db is older, migrations needed *)
        DB.with_transaction (fun () ->
            let migrations =
              find_migrations_to_run ~current_version t.migrations
            in
            (* first, create any tables that aren't present yet *)
            let* () =
              Hashtbl.to_seq t.tables |> List.of_seq
              |> List.sort (fun (k1, _) (k2, _) -> Int.compare k1 k2)
              |> Lwt_seq.of_list
              |> Lwt_seq.map_s
                   (fun (_, MkTable (_, name, since, _, table, constraints)) ->
                     match since with
                     | Some since when compare_version current_version since < 0
                       ->
                         let table_def =
                           Schema.to_sql ~name table constraints
                         in
                         let req =
                           Caqti_request.Infix.(
                     ...
                     | _ ->
                         (* if since is not present, or current version is
                            ...
                            already present *)
                         Lwt.return_ok ())
              |> Lwt_seq.to_list |> Lwt.map result_all_unit
            in
            ...
            let* () =
              Lwt_list.map_s
                (fun (_, migrations) ->
                  Lwt_list.map_s
                    (fun migration -> DB.exec migration ())
                    migrations
                  |> Lwt.map result_all_unit)
                migrations
              |> Lwt.map result_all_unit
            ...
            let* () =
              ...
              |> Lwt_seq.of_list
              |> Lwt_seq.map_s
                   (fun (_, MkTable (_, _, since, migrations, _, _)) ->
                     match since with
                     | Some since when compare_version current_version since < 0
                       ->
                         Lwt.return_ok ()
                     | _ ->
                         ...
                         let migrations =
                           find_migrations_to_run ~current_version migrations
                         in
                         (* run them in order *)
                         Lwt_list.map_s
                           (fun (_, migrations) ->
