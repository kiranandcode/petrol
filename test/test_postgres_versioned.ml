...
module Bookmark = struct
  ...
  module V1 = struct
    let db = Petrol.VersionedSchema.init_postgres v_1_0_0 ~name:"bookmark" 
  ...
  module V1_2  = struct
    let db = Petrol.VersionedSchema.init_postgres v_1_2_0 ~name:"bookmark" 
  ...
  module V2 = struct
    let db = Petrol.VersionedSchema.init_postgres v_2 ~name:"bookmark" 
    ...
    module Tags = struct
      ...
      let t, Expr.[_tag_id;bookmark_id; tag_text] =
        VersionedSchema.declare_table db ~name:"tag" ~since:v_2
          Schema.[
            field ~constraints:[primary_key ()] "tag_id" ~ty:Type.smallserial;
...
let () =
  Test_utils.main_postgres begin fun conn -> fun args ->
    let[@warning "-8"] ver :: args = args in
